...
from . import analyze_problems, fetch_problems
...
def process_fetch(args: argparse.Namespace) -> tuple[int, str | None]:
    ...
    Returns:
        Tuple of (exit_code, file_path) where file_path is the path to the file
        to analyze.
    """
    ...
    latest_file = max(
        json_files,
        key=lambda f: os.path.getmtime(os.path.join(fetch_problems.OUTPUT_DIR, f)),
    )
...
def get_file_to_analyze(args: argparse.Namespace) -> tuple[int, str | None]:
    ...
    latest_file = max(
        json_files,
        key=lambda f: os.path.getmtime(os.path.join(analyze_problems.RAW_DATA_DIR, f)),
    )
...
def validate_args(args: argparse.Namespace) -> tuple[bool, str | None]:
    ...
    Returns:
        Tuple of (is_valid, error_message). If is_valid is False, error_message contains
        the reason.
    """
    if args.fetch_only and args.analyze_only:
        return False, "Cannot specify both --fetch-only and --analyze-only"

    if args.file and not args.analyze_only:
        return False, "--file can only be used with --analyze-only"

    return True, None


def select_file_to_analyze(args: argparse.Namespace) -> tuple[int, str | None]:
    """Select the file to analyze based on command line arguments.

    Args:
        args: Parsed command line arguments.

    Returns:
        Tuple of (exit_code, file_path) where file_path is the path to analyze.
    """
    if not args.analyze_only:
        ...
        # Analyze only mode
        return get_file_to_analyze(args)
...
def main() -> int:
    ...
    parser.add_argument(
        "--fetch-only",
        action="store_true",
        help="Only fetch Reddit posts, don't analyze them.",
    )
    parser.add_argument(
        "--analyze-only",
        action="store_true",
    ...
    try:
        ...
        # Get file to analyze
        exit_code, file_to_analyze = select_file_to_analyze(args)
        if exit_code != 0 or file_to_analyze is None:
            return exit_code

        # Load OpenAI credentials and initialize client
        credentials = analyze_problems.load_env_vars()
        client = analyze_problems.initialize_openai_client(credentials)

        # Analyze posts
        print(f"Analyzing posts in {file_to_analyze}...")
        output_path = analyze_problems.analyze_file(file_to_analyze, client=client)
